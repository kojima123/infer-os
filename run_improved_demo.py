"""
改良版NPUデモ実行スクリプト
継続的生成ループとDirectML最適化を実装

使用方法:
    python run_improved_demo.py --interactive
    python run_improved_demo.py --prompt "人参について教えてください。"
"""
...
# 改良版NPUエンジンのインポート
try:
    from improved_npu_engine import ImprovedNPUEngine
    IMPROVED_NPU_AVAILABLE = True
except ImportError as e:
    print(f"⚠️ 改良版NPUエンジンが利用できません: {e}")
    IMPROVED_NPU_AVAILABLE = False

class ImprovedNPUDemo:
    """改良版NPUデモ"""
    ...
    def setup(self) -> bool:
        ...
        try:
            ...
            # 改良版NPUエンジンセットアップ
            if IMPROVED_NPU_AVAILABLE:
                print("🚀 改良版NPUエンジンセットアップ中...")
                self.npu_engine = ImprovedNPUEngine(self.model, self.tokenizer)
                
                if self.npu_engine.setup_npu():
                    print("✅ 改良版NPUエンジンセットアップ完了")
                    print(f"🎯 使用アプローチ: {self.npu_engine.npu_approach}")
                else:
                    print("⚠️ 改良版NPUエンジンセットアップ失敗、CPU推論を使用")
                    self.npu_engine = None
            else:
                print("⚠️ 改良版NPUエンジンが利用できません、CPU推論を使用")
        ...
        try:
            ...
            if self.npu_engine and self.npu_engine.is_npu_ready:
                ...
                if "error" not in result:
                    ...
                    print(f"⚠️ 改良版NPU推論エラー: {result['error']}")
                    print("🔄 CPU推論にフォールバック")
    ...
    def run_interactive(self):
        """インタラクティブモード"""
        print("\n🇯🇵 改良版NPUデモ - インタラクティブモード")
        print("🎯 継続的生成ループ + DirectML最適化版")
    ...
    def _show_help(self):
        ...
        print("\n🎯 改良版NPUアプローチ:")
        print("  - directml_optimized: DirectML最適化 + 継続的生成ループ")
        print("  - continuous_load: 継続的NPU負荷生成 + CPU推論")
        print("\n💡 改善点:")
        print("  - 継続的生成ループ実装（複数トークン生成）")
        print("  - DirectML最適化（NPU負荷率向上）")
        print("  - 実モデル重み使用（高精度推論）")
        print("\n📊 NPU負荷確認:")
        print("  - タスクマネージャーでNPU使用率を確認してください")
        print("  - 改良版では継続的にNPU負荷率が上昇します")
...
def main():
    """メイン関数"""
    parser = argparse.ArgumentParser(description="改良版NPUデモ")
    ...
    # デモ初期化
    demo = ImprovedNPUDemo(args.model)
    ...
    try:
        ...
        if args.interactive:
            ...
            print("例: python run_improved_demo.py --interactive")
            print("例: python run_improved_demo.py --prompt \"人参について教えてください。\"")
