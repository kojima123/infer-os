# -*- coding: utf-8 -*-
"""
Ryzen AI NPU対応日本語LLMシステム（Windows権限エラー修正版）
kyo-takano/open-calm-7b-8bit モデル使用
Windows環境でのシンボリックリンク権限問題を解決
"""
...
class RyzenAIJapaneseLLMSystem:
    ...
    def __init__(self, enable_infer_os: bool = False):
        ...
        print(f"🛠️ Windows権限問題対応: シンボリックリンク回避")
    ...
    def download_model(self) -> bool:
        ...
        try:
            ...
            self.model_dir = self.cache_dir / "open-calm-7b-8bit"
            ...
            while self.npu_monitoring:
                try:
                    ...
                    if self.active_provider == 'VitisAIExecutionProvider':
                        ...
                        current_usage = min(cpu_usage * 0.4, 100.0)
                    elif self.active_provider == 'DmlExecutionProvider':
                        # DML使用時はCPU使用率の一部をGPU使用率として推定
                    ...
                    if len(self.npu_usage_history) > 1:
                        ...
                        if abs(current_usage - prev_usage) >= 1.0:
                            if current_usage > 5.0:  # 5%以上の使用率時のみ
                                print(f"🔥 {self.active_provider} 使用率変化: {prev_usage:.1f}% → {current_usage:.1f}%")
                                if self.active_provider in ['VitisAIExecutionProvider', 'DmlExecutionProvider']:
    ...
    def generate_text_onnx(self, prompt: str, max_new_tokens: int = 50) -> str:
        ...
        try:
            ...
            print(f"💬 ONNX {self.active_provider} 日本語生成: '{prompt}'")
            ...
            return f"❌ ONNX {self.active_provider} 生成エラー: {e}"
    
    def run_benchmark(self, num_inferences: int = 30) -> Dict[str, Any]:
        """NPUベンチマーク実行"""
        try:
            print(f"📊 {self.active_provider} ベンチマーク開始: {num_inferences}回推論")
            print(f"🎯 モデル: {self.model_id}")
            ...
            if npu_stats:
                print(f"  🔥 最大{self.active_provider}使用率: {npu_stats['max_npu_usage']:.1f}%")
                print(f"  📊 平均{self.active_provider}使用率: {npu_stats['avg_npu_usage']:.1f}%")
                print(f"  🎯 {self.active_provider}動作率: {npu_stats['npu_activity_rate']:.1f}%")
    ...
    def interactive_mode(self):
        ...
        print(f"📝 モデル: {self.model_id}")
        print(f"🔧 プロバイダー: {self.active_provider}")
        print(f"💡 コマンド: 'quit'で終了, 'status'で状況確認")
        print(f"=" * 60)
        ...
        try:
            while True:
                ...
                if prompt.lower() == 'status':
                    npu_stats = self.get_npu_statistics()
                    if npu_stats:
                        print(f"🔥 {self.active_provider} 統計:")
    ...
    def initialize_system(self) -> bool:
        ...
        try:
            ...
            if not self.export_to_onnx():
                print("⚠️ ONNX エクスポート失敗、PyTorchモードで継続")
...
def main():
    parser = argparse.ArgumentParser(description="Ryzen AI NPU対応日本語LLMシステム（Windows権限エラー修正版）")
